"""
End-to-end SWE training pipeline for CPU.
Combines extraction, PCA, and distillation into one script.
"""
import torch
import os
import numpy as np
...
def evaluate_embeddings(word2vec, tokenizer):
    ...
    for w1, w2, should_be_similar in test_pairs:
        if w1 in word2vec and w2 in word2vec:
            ...
            status = "✓" if (sim > 0.3) == should_be_similar else "✗"
...
def main():
    ...
    parser.add_argument('-teacher', type=str, default='all-MiniLM-L6-v2', help='Teacher model')
    ...
    parser.add_argument('-output', type=str, default='data/output/swe.txt', help='Output path')
    args = parser.parse_args()

    print("=" * 60)
    print("SWE4SEMANTICS: Full Training Pipeline")
    print("=" * 60)
    ...
    word2vec, tokenizer = extract_embeddings(word2sent, args.model, nsent=args.nsent)
    ...
    word2vec = train_distillation(word2vec, tokenizer, args.teacher, epochs=args.epochs, max_steps=args.max_steps)
